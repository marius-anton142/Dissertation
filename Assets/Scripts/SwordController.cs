...
public class SwordController : MonoBehaviour
{
    ...
    private Vector3 aiAimDirection = Vector3.right;

    public void AttackInDirection(Vector3 attackDirection)
    {
    ...
    {
        return weaponState == WeaponState.Equipped &&
               !isAttacking &&
    ...
    public void SetAIControl(bool value)
    {
        ...
        if (value)
        {
            ...
            {
                aiAimDirection = transform.localPosition.normalized;
                direction = aiAimDirection;
            }
            else if (direction != Vector3.zero)
            {
                aiAimDirection = direction.normalized;
            }
        }
    }

    private void ApplyWeaponDirection()
    {
        ...
        transform.localRotation = Quaternion.AngleAxis(adjustedAngle + angleOffset, Vector3.forward);
    }

    public IEnumerator AimAndAttackInDirection(Vector3 attackDirection, float aimDuration)
    {
        if (weaponState != WeaponState.Equipped)
        {
            yield break;
        ...
        Vector3 startDirection = aiAimDirection.sqrMagnitude > 0.001f
            ? aiAimDirection.normalized
            : Vector3.right;

        Vector3 targetDirection = attackDirection.normalized;
        ...
        while (elapsed < aimDuration)
        {
            ...
            aiAimDirection = Vector3.Slerp(startDirection, targetDirection, t).normalized;
            direction = aiAimDirection;
            ApplyWeaponDirection();

            yield return null;
        }

        aiAimDirection = targetDirection;
        direction = aiAimDirection;
        ApplyWeaponDirection();
    ...
    private void Awake()
    {
        ...
        if (WeaponDot == null)
        {
            WeaponDot = GameObject.Find("WeaponDot");
        }
