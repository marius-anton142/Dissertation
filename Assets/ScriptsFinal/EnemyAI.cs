...
public class EnemyAI : MonoBehaviour
{
    ...
    private bool IsPlayerOnKnightAttackTile(Vector2Int knightCell, Vector2Int playerCell)
    {
        int dx = Mathf.Abs(playerCell.x - knightCell.x);
        int dy = Mathf.Abs(playerCell.y - knightCell.y);
    ...
    private void NextStep()
    {
        ...
        if (type == "Knight")
        {
            if (IsPlayerOnKnightAttackTile(currentCell, targetCell) && !isLeaping)
            {
