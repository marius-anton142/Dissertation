...
public class WeaponDotScript : MonoBehaviour
{
    ...
    {
        return slots.IndexOf(slotSelected);
    }

    public void SelectSlotForAI(int slotIndex)
    {
        ...
        if (slotIndex == GetSelectedSlotIndex())
        {
    ...
    public bool AddWeaponToSlotForExperiment(GameObject newWeapon, Sprite itemSprite, int slotIndex, Vector2 maxSize = default)
    {
        if (maxSize == default)
        {
            maxSize = new Vector2(16f, 24f);
        }

        if (slotIndex < 0 || slotIndex >= weaponSlots.Length)
        {
            Debug.LogWarning("Invalid experiment weapon slot index: " + slotIndex);
            return false;
        }

        if (weaponSlots[slotIndex] != null)
        {
            Debug.LogWarning("Experiment weapon slot is already occupied: " + slotIndex);
            return false;
        }
