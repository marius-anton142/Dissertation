...
public enum AIExperimentMethod
{
    PromptOnly,
    PromptMemory,
    PromptMemoryGraph,
    PromptMemoryGraphRag,
    OracleCustom
}
...
public class DungeonGenerationScript01 : MonoBehaviour
{
    ...
    public int dungeonSeed = 1001;

    public bool useExperimentLayout = true;
    public int experimentCaseId = 1;

    public bool experimentCobwebsEnabled = false;
    [Range(0f, 1f)]
    public float experimentCobwebRoomChance = 0.5f;
    public float experimentCobwebChancePerFloorTile = 0.05f;

    public GameObject AxePrefab01;
    public AIExperimentMethod experimentMethod = AIExperimentMethod.PromptMemoryGraph;
    private List<GameObject> experimentAxeObjects = new List<GameObject>();
    ...
    private List<Vector3Int> doorsHorizontalOpenInRoom = new List<Vector3Int>();
    private List<Vector3Int> doorsHorizontalOpenInRoomWidth1 = new List<Vector3Int>();
    private List<Vector3Int> doorsVerticalOpenInRoom = new List<Vector3Int>();
    private List<Vector3Int> doorsVerticalOpenInRoomWidth1 = new List<Vector3Int>();
    ...
    public bool aiDoorOpenedThisStep = false;
    public Vector3Int aiLastOpenedDoorPosition;
    public WeaponDotScript weaponInventory;
    public int aiDoorsOpenedCount = 0;
    ...
    private void Awake()
    {
        if (useFixedSeed)
        {
            ...
            Debug.Log("Dungeon seed: " + dungeonSeed);
        }
    }

    public int GetTreeCount()
    {
        return treesInRoom.Count;
    }

    public int GetCobwebCount()
    {
        return cobwebsInRoom.Count;
    }

    public bool IsAxeAtPosition(Vector3Int position)
    {
        foreach (GameObject axe in experimentAxeObjects)
        {
            if (axe == null || !axe.activeInHierarchy)
            {
                continue;
            }

            SwordController swordController = axe.GetComponent<SwordController>();

            if (swordController != null && swordController.weaponState != WeaponState.Free)
            {
                continue;
            }

            Vector3Int axeCell = new Vector3Int(
                Mathf.FloorToInt(axe.transform.position.x),
                Mathf.FloorToInt(axe.transform.position.y),
                0
            );
    ...
    private void GiveAxeToPlayerForExperiment()
    {
        if (AxePrefab01 == null)
        {
            Debug.LogWarning("AxePrefab01 is not assigned.");
            return;
        }

        if (weaponInventory == null)
        {
            Debug.LogWarning("Weapon inventory is not assigned.");
            return;
        }

        Vector3 playerPosition = weaponInventory.player.position;

        GameObject axe = Instantiate(AxePrefab01, playerPosition, Quaternion.identity);
        experimentAxeObjects.Add(axe);

        if (experimentAxeObjects.Count == 1)
        {
            axeObject = axe;
        }

        SpriteRenderer spriteRenderer = axe.GetComponent<SpriteRenderer>();

        if (spriteRenderer == null)
        {
            Debug.LogWarning("Axe has no SpriteRenderer.");
            return;
        }

        bool added = weaponInventory.AddWeaponToSlotForExperiment(axe, spriteRenderer.sprite, 0);

        if (!added)
        {
            Debug.LogWarning("Could not add axe to inventory for experiment.");
        }
    }

    public Room GetRoomAtWorldCell(Vector3Int worldCell)
    {
        foreach (Room room in rooms)
        {
            foreach (Vector3Int localTile in room.FloorTileCoordinates)
            {
                Vector3Int worldTile = room.GetPosition() + localTile;

                if (worldTile == worldCell)
                {
                    return room;
                }
            }
    ...
    public int GetGeneratedRoomCount()
    {
        return rooms.Count;
    }

    public List<string> BuildRoomMapForAI(Room room, Vector3Int playerCell)
    {
        ...
        foreach (Vector3Int localTile in room.FloorTileCoordinates)
        {
            floorTiles.Add(room.GetPosition() + localTile);
        }

        int minX = floorTiles.Min(tile => tile.x) - 1;
        int maxX = floorTiles.Max(tile => tile.x) + 1;
        int minY = floorTiles.Min(tile => tile.y) - 1;
        int maxY = floorTiles.Max(tile => tile.y) + 1;
        ...
        for (int y = maxY; y >= minY; y--)
        {
            ...
            for (int x = minX; x <= maxX; x++)
            {
                ...
                if (!IsInCurrentRoomOrAdjacent(cell, floorTiles))
                {
                    row += "#";
                    continue;
                }
                ...
                else if (IsTreeAtPosition(cell))
                {
                    row += "T";
                }
                else if (IsAxeAtPosition(cell))
                {
                    row += "A";
                }
                else if (IsDoorAtPosition(cell))
                {
                    row += "D";
                }
                else if (IsDoorOpenAtPosition(cell))
                {
                    row += "Z";
                }
                else if (IsCobwebAtPosition(cell))
                {
                    row += "C";
                }
                else if (floorTiles.Contains(cell))
                {
                    row += "O";
                }
                else
    ...
    private bool IsInCurrentRoomOrAdjacent(Vector3Int cell, HashSet<Vector3Int> floorTiles)
    {
        if (floorTiles.Contains(cell))
        {
            return true;
        }

        return floorTiles.Contains(cell + Vector3Int.up) ||
               floorTiles.Contains(cell + Vector3Int.down) ||
               floorTiles.Contains(cell + Vector3Int.left) ||
               floorTiles.Contains(cell + Vector3Int.right);
    }

    public Vector2Int GetRoomMapOriginForAI(Room room)
    {
        HashSet<Vector3Int> floorTiles = new HashSet<Vector3Int>();
    ...
    public string GetExperimentMethodName()
    {
        switch (experimentMethod)
        {
            ...
            case AIExperimentMethod.PromptMemoryGraph:
                return "prompt_memory_graph";

            case AIExperimentMethod.PromptMemoryGraphRag:
                return "prompt_memory_graph_rag";
    ...
    public void PlaceCobwebAtPlayerPosition()
    {
        if (player == null)
        {
            Debug.LogWarning("Cannot place cobweb on player because player is not assigned.");
            return;
        }

        Vector3Int playerCell = new Vector3Int(
            Mathf.FloorToInt(player.transform.position.x) + 1,
            Mathf.FloorToInt(player.transform.position.y),
            0
        );

        PlaceCobwebAtPosition(playerCell);
    }

    public void PlaceCobwebAtPosition(Vector3Int worldTile)
    {
        if (CobwebPrefab01 == null)
        {
            Debug.LogWarning("Cannot place cobweb because CobwebPrefab01 is not assigned.");
            return;
        }

        if (tilemapFloor.GetTile(worldTile) == null)
        {
            Debug.LogWarning("Cannot place cobweb because target tile is not floor: " + worldTile);
            return;
        }

        if (IsCobwebAtPosition(worldTile))
        {
            return;
        }
        ...
        PlaceObject(
            ...
            selectedCobwebSprite
        );

        cobwebsInRoom.Add(worldTile);

        Debug.Log("Placed experiment cobweb at: " + worldTile);
    }
    ...
    public void FillRoomWithRandomCobwebsForExperiment(Room room)
    {
        if (!experimentCobwebsEnabled)
        {
            return;
        }

        if (Random.value > experimentCobwebRoomChance)
        {
            return;
        }
        ...
        foreach (Vector3Int localTile in room.FloorTileCoordinates)
        {
            ...
            if (Random.value > experimentCobwebChancePerFloorTile)
            {
                continue;
            }

            if (IsEntityAtPosition(worldTile))
            {
                continue;
            }

            if (IsTreeAtPosition(worldTile))
            {
                continue;
            }

            if (IsAxeAtPosition(worldTile))
            {
                continue;
            }

            if (IsDoorAtPosition(worldTile) || IsDoorOpenAtPosition(worldTile))
            {
                continue;
            }
            ...
            Sprite selectedCobwebSprite = CobwebPrefab01.GetComponent<SpriteScript>().GetRandomSprite(rotate: true);
            PlaceObject(worldTile, CobwebPrefab01, new Vector3(0.5f, 0.5f, 0), selectedCobwebSprite);
            cobwebsInRoom.Add(worldTile);
        }
    }
    ...
    {
        if (IsDoorHorizontalWidth1AtPosition(position))
        {
            doorsHorizontalInRoomWidth1.Remove(position);
            doorsHorizontalOpenInRoomWidth1.Add(position);
        }
        else
        {
            if (IsDoorHorizontalAtPosition(position + new Vector3Int(-1, 0, 0)))
            {
                doorsHorizontalInRoom.Remove(position + new Vector3Int(-1, 0, 0));
                doorsHorizontalOpenInRoom.Add(position + new Vector3Int(-1, 0, 0));
            }
            else if (IsDoorHorizontalAtPosition(position))
            {
                doorsHorizontalInRoom.Remove(position);
                doorsHorizontalOpenInRoom.Add(position);
            }
        }
    }
    ...
    {
        if (IsDoorVerticalWidth1AtPosition(position))
        {
            doorsVerticalInRoomWidth1.Remove(position);
            doorsVerticalOpenInRoomWidth1.Add(position);
        }
        else
        {
            if (IsDoorVerticalAtPosition(position + new Vector3Int(0, -1, 0)))
            {
                doorsVerticalInRoom.Remove(position + new Vector3Int(0, -1, 0));
                doorsVerticalOpenInRoom.Add(position + new Vector3Int(0, -1, 0));
            }
            else if (IsDoorVerticalAtPosition(position))
            {
                doorsVerticalInRoom.Remove(position);
                doorsVerticalOpenInRoom.Add(position);
            }
        }
    }
    ...
    {
        if (!IsDoorAtPosition(position))
        {
            return;
        }

        GameObject doorObject = IdentifyDoorAtPosition(position);
        ...
        if (doorObject != null)
        {
            ...
            doorScript.DecreaseHp();

            if (doorScript.GetHp() == -1)
            {
                ...
                aiDoorOpenedThisStep = true;
                aiLastOpenedDoorPosition = position;
            }
        }
    }
    ...
    public bool IsTreeAtPosition(Vector3Int position)
    {
    ...
    public bool IsStoneAtPosition(Vector3Int position)
    {
    ...
    public bool IsDoorHorizontalWidth1AtPosition(Vector3Int position)
    {
    ...
    public bool IsDoorVerticalWidth1AtPosition(Vector3Int position)
    {
        foreach (var doorPos in doorsVerticalInRoomWidth1)
        {
            if (doorPos == position)
    ...
    public bool IsDoorAtPosition(Vector3Int position)
    {
        return (IsDoorHorizontalAtPosition(position) || IsDoorVerticalAtPosition(position));
    }

    public bool IsDoorHorizontalOpenAtPosition(Vector3Int position)
    {
        foreach (var doorPos in doorsHorizontalOpenInRoom)
        {
            if (doorPos == position)
            {
                return true;
        ...
        foreach (var doorPos in doorsHorizontalOpenInRoomWidth1)
        {
            if (doorPos == position)
            {
                return true;
    ...
    public bool IsDoorHorizontalOpenAtPositionAny(Vector3Int position)
    {
        Vector3Int left = position + Vector3Int.left;
    ...
    public bool IsDoorVerticalOpenAtPosition(Vector3Int position)
    {
        foreach (var doorPos in doorsVerticalOpenInRoom)
        {
            if (doorPos == position)
            {
                return true;
        ...
        foreach (var doorPos in doorsVerticalOpenInRoomWidth1)
        {
            if (doorPos == position)
            {
                return true;
    ...
    public bool IsDoorVerticalOpenAtPositionAny(Vector3Int position)
    {
        Vector3Int down = position + Vector3Int.down;
    ...
    public bool IsDoorOpenAtPositionAny(Vector3Int position)
    {
        return (IsDoorHorizontalOpenAtPositionAny(position) || IsDoorVerticalOpenAtPositionAny(position));
    }

    public bool IsDoorOpenAtPosition(Vector3Int position)
    {
        return (IsDoorHorizontalOpenAtPosition(position) || IsDoorVerticalOpenAtPosition(position));
    }
    ...
    private void Start()
    {
        ...
        if (useExperimentLayout)
        {
            ApplyExperimentLayout(initialRoom);
        }
        ...
        if (experimentCobwebsEnabled)
        {
            foreach (Room room in rooms)
            {
                FillRoomWithRandomCobwebsForExperiment(room);
            }
        }
    ...
    private void ApplyExperimentLayout(Room startRoom)
    {
        if (axeObject != null)
        {
        ...
        treesInRoom.Clear();
        experimentAxeObjects.Clear();

        List<Room> nonStartRooms = GetNonStartRooms(startRoom);

        switch (experimentCaseId)
        {
            case 1:
                PlaceTreeInRoom(startRoom);
                GiveAxeToPlayerForExperiment();
                break;

            case 2:
                PlaceAxeInRoom(startRoom);
                PlaceTreeInRoom(startRoom);
                break;
            ...
            case 3:
                ...
                foreach (Room room in nonStartRooms)
                {
                    PlaceTreeInRoom(room);
                }
                break;
            ...
            case 5:
                if (nonStartRooms.Count >= 2)
                {
                    List<Room> shuffledRooms = new List<Room>(nonStartRooms);
                    ShuffleList(shuffledRooms);

                    PlaceAxeInRoom(shuffledRooms[0]);
                    PlaceTreeInRoom(shuffledRooms[1]);
                }
                else if (nonStartRooms.Count == 1)
                {
                    PlaceAxeInRoom(nonStartRooms[0]);
                    PlaceTreeInRoom(startRoom);
                }
                break;

            default:
                Debug.LogWarning("Unknown experiment case id: " + experimentCaseId);
                break;
        }

        Debug.Log("Applied experiment layout case: " + experimentCaseId);
    }

    private void PlaceTreeInRoom(Room room)
    {
        Vector3Int? freePosition = GetRectanglesInRoomFree(room, 1, 1);

        if (freePosition == null)
        {
            Debug.LogWarning("Could not place tree in room.");
            return;
        }

        Vector3Int worldPosition = freePosition.Value + room.GetPosition();

        if (IsEntityAtPosition(worldPosition) || IsEntityAtPosition(worldPosition + Vector3Int.up))
        {
            Debug.LogWarning("Tree position blocked in room.");
            return;
        }

        PlaceObject(worldPosition, TreePrefab01, new Vector3(0.5f, 0.5f, 0));
        treesInRoom.Add(worldPosition);
    }

    private GameObject PlaceAxeInRoom(Room room)
    {
        if (AxePrefab01 == null)
        {
            Debug.LogWarning("AxePrefab01 is not assigned.");
        ...
        if (freePosition == null)
        {
            Debug.LogWarning("Could not place axe in room.");
            return null;
        }

        Vector3Int worldPosition = freePosition.Value + room.GetPosition();

        if (IsEntityAtPosition(worldPosition))
        {
            Debug.LogWarning("Axe position blocked in room.");
            return null;
        }

        GameObject axe = PlaceObject(worldPosition, AxePrefab01, new Vector3(0.5f, 0.5f, 0));
        experimentAxeObjects.Add(axe);
        ...
        Debug.Log("Placed axe in room " + GetRoomId(room) + " at " + worldPosition);

        return axe;
    }

    private List<Room> GetNonStartRooms(Room startRoom)
    {
